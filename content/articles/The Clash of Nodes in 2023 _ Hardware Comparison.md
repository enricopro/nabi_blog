...
![](https://miro.medium.com/v2/resize:fit:786/format:webp/1*Ar7d-BvojT_5IGtLkRTE-A.png)
...
Cubit Pro has a much greater height due to the capacity to hold two SSD disks and a mini UPS as we will see later.
...
![](https://miro.medium.com/v2/resize:fit:1100/0*kC8c7xcvzQza0qHv)
...
As you can see from the graph, the range of performance is really substantial, from the Embassy One to the Cubit Pro, which is about 14 times more powerful.
...
![](https://miro.medium.com/v2/resize:fit:1100/0*vQPCKComn2CXULrI)

In the low-range we find the Model 2 and the Embassy One equipped with 8GB of RAM, whereas the Tanto has only 4 GB of RAM.

For the high end, both the Embassy Pro and Cubit Pro are equipped with 32GB of RAM, but only Cubit Pro offers Error Correction Code (ECC) compatibility, a crucial feature for maintaining high system stability and preventing data corruption over long periods of time (ever heard about [bit rot](https://en.wikipedia.org/wiki/Data_degradation)?).  
...
Below is the graph showing the maximum amount of storage with which the 5 different devices are provided. Cubit Pro is the only one that can mount two SSDs, possibly in RAID configuration.

![](https://miro.medium.com/v2/resize:fit:1100/0*mHCGM872oKiBNvLM)
...
The Tanto, the Embassy Pro and Cubit Pro can use NVMe disks that are much more performing, although the RockPro64 (Tanto’s board) cannot take advantage of these disks at 100 percent.

![](https://miro.medium.com/v2/resize:fit:1100/0*q74qzqdqGPz8k0tA)
...
*Note: Of the two disposable disk ports on Cubit Pro, only one is m2, while the other is SATA III (much slower). If you choose to use Cubit Pro with a SATA RAID of drives, the speed will be limited by the SATA interface to 550/500 MB/s read/write on both disks.*
...
The Model 2, Embassy Pro and Cubit Pro offer both VT-x and VT-d (or the AMD equivalents).
...
- Cubit Pro [Ryzen Embedded v2516](https://en.wikichip.org/wiki/amd/ryzen_embedded/v2516)
...
The Tanto, Model 2 and especially the Embassy Pro cannot take advantage of the speed of their respective disks because the data transfer rate is limited by the antiquated 1Gb Ethernet port. Cubit Pro differs from the others by having two Ethernet ports, one of which is 2.5Gb.

![](https://miro.medium.com/v2/resize:fit:1100/0*Jlrt1E__qcKlOISU)
...
![](https://miro.medium.com/v2/resize:fit:640/format:webp/1*1osv_9LkcWDI5_oTjQSJnw.png)
...
Cubit Pro is by far the most stable and reliable thanks to the:
...
![](https://miro.medium.com/v2/resize:fit:582/format:webp/1*Ay1dBcL1bUAmlmvrXOr6Xw.png)
...
![](https://miro.medium.com/v2/resize:fit:640/format:webp/1*Dm5uXHTm6o5FxyStuR-YWw.png)

The prices of the various devices make it clear that we are comparing two different ranges, i.e., low end as far as the Embassy One, Tanto and Model 2 are concerned, while the Embassy Pro and Cubit Pro fall in the high end of personal servers.
...
![](https://miro.medium.com/v2/resize:fit:640/format:webp/1*OgKgLxclkAiOJ7W8Q_Lgfg.png)

The choice here is very straightforward. For a difference of only $200 or +11%, Cubit Pro is more powerful, more flexible, and most importantly, more reliable than the Embassy Pro.

To recap, Cubit Pro has:
...
We at Nabi aspire to *excellence* in designing our products for the self-sovereign individual. We believe Cubit Pro lives up to our mission, and reflects what a personal server should be.
